...
class rvFSA : public rvWeapon {
public:

	CLASS_PROTOTYPE(rvFSA);
...
private:
	...
	CLASS_STATES_PROTOTYPE(rvFSA);
};

CLASS_DECLARATION(rvWeapon, rvFSA)
END_CLASS
...
================
*/
void rvFSA::Save(idSaveGame* savefile) const {
}
...
================
*/
void rvFSA::Restore(idRestoreGame* savefile) {
	hitscans = spawnArgs.GetFloat("hitscans");
}
...
CLASS_STATES_DECLARATION(rvFSA)
STATE("Idle", rvFSA::State_Idle)
STATE("Fire", rvFSA::State_Fire)
STATE("Reload", rvFSA::State_Reload)
END_CLASS_STATES
...
================
*/
stateResult_t rvFSA::State_Idle(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
	};
...
================
*/
stateResult_t rvFSA::State_Fire(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
	};
	...
	case STAGE_INIT:
		...
		Attack(false, hitscans, spread, 0, 0.8f);
...
================
*/
stateResult_t rvFSA::State_Reload(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
